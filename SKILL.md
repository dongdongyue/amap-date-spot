---
name: amap-date-spot
display_name: amap-date-spot 约会地点推荐 Date Spot
...
生成两个 CSV，保留完整搜索结果便于复查（文字/HTML 只展示精选内容）：

`date_plan.csv` — 动线行程表（精简，和文字报告一致）：
```csv
...
`date_pois.csv` — **完整搜索结果**（所有搜到的 POI，10+ 行）：
```csv
类别,场所名,地址,人均,评分,是否推荐,推荐理由
咖啡馆,星巴克臻选(新百1F),中山南路1号,¥30,4.6,✅,商圈核心·第一次见面不出错
咖啡馆,Manner Coffee,中山南路1号,¥20,4.5,❌,
茶馆,六初茶社,正洪街18号F22,¥85,4.1,✅,22楼安静私密
...
```

CSV 保留全量数据——候选池里的所有 POI 都写入，方便用户自己筛选。文字报告和 HTML 只展示精选的 3 个推荐。
